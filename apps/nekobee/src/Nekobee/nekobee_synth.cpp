...
/*
 * nekobee_synth_select_program
 */
void
nekobee_synth_select_program(nekobee_synth_t *synth, unsigned long bank,
                            unsigned long program)
{
    if (bank || program >= 128) return;
    synth->current_program = program;
    nekobee_voice_set_ports(synth, &synth->patches[program]);
}

/*
 * nekobee_data_friendly_patches
 *
 * give the new user a default set of good patches to get started with
 */
void
nekobee_data_friendly_patches(nekobee_synth_t *synth)
{
...
/*
 * nekobee_synth_set_program_descriptor
 */
int
nekobee_synth_set_program_descriptor(nekobee_synth_t *synth,
                                    ...
                                    unsigned long bank, unsigned long program)
{
    ...
    pd->Bank = 0;
    pd->Program = 0;
    pd->Name = "none"; // needs some kind of name in the descriptor
    return 1;
 ...
 */
void
nekobee_synth_render_voices(nekobee_synth_t *synth, LADSPA_Data *out, unsigned long sample_count,
