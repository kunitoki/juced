...
//==============================================================================
void Distorsion::applyfilters (REALTYPE *efxoutl, REALTYPE *efxoutr, int numSamples)
{
    lpfl->filterout (efxoutl, numSamples);
    hpfl->filterout (efxoutl, numSamples);
    if (Pstereo != 0) // stereo
    {
        lpfr->filterout (efxoutr, numSamples);
        hpfr->filterout (efxoutr, numSamples);
    }
}

//==============================================================================
void Distorsion::out (REALTYPE *smpsl, REALTYPE *smpsr, const int numSamples)
{
    int i;
    ...
    // pre filtering
    if (Pprefiltering != 0) applyfilters (efxoutl, efxoutr, numSamples);

    // no optimised, yet (no look table)
    waveshapesmps (numSamples, efxoutl, Ptype + 1, Pdrive);
    if (Pstereo != 0) waveshapesmps (numSamples, efxoutr, Ptype + 1, Pdrive);

    // post filtering
    if (Pprefiltering == 0) applyfilters (efxoutl, efxoutr, numSamples);

    // stereo mix
    if (Pstereo == 0) for (i = 0; i < numSamples; i++) efxoutr[i] = efxoutl[i];
...
//==============================================================================
void Distorsion::setParameter (int npar, uint8 value)
{
    switch (npar)
    {
    ...
    case 2: setlrcross (value);
            break;
    case 3: setdrive (value);
            break;
    case 4: setlevel (value);
            break;
...
uint8 Distorsion::getParameter (int npar)
{
    switch (npar)
    {
    case 0: return(Pvolume);        break;
    case 1: return(Ppanning);       break;
    case 2: return(Plrcross);       break;
    case 3: return(Pdrive);         break;
    case 4: return(Plevel);         break;
    case 5: return(Ptype);          break;
    case 6: return(Pnegate);        break;
    case 7: return(Plpf);           break;
    case 8: return(Phpf);           break;
    case 9: return(Pstereo);        break;
    case 10:return(Pprefiltering);  break;
    }
    return (0); //in case of bogus parameter number
}

//==============================================================================
void Distorsion::setvolume (uint8 Pvolume)
{
...
void Distorsion::setpanning (uint8 Ppanning)
{
    this->Ppanning = Ppanning;
    panning = (Ppanning + 0.5) / 127.0;
}

void Distorsion::setdrive (uint8 Pdrive)
{
    this->Pdrive = Pdrive;
    inputvol = pow (5.0, (Pdrive - 32.0) / 127.0);
    if (Pnegate != 0) inputvol *= -1.0;
}

void Distorsion::setlevel (uint8 Plevel)
{
    this->Plevel = Plevel;
    level = dB2rap (60.0 * Plevel / 127.0 - 40.0);
}

void Distorsion::setlrcross (uint8 Plrcross)
{
    this->Plrcross = Plrcross;
    lrcross = Plrcross / 127.0 * 1.0;
}

void Distorsion::setlpf (uint8 Plpf)
{
    this->Plpf = Plpf;
...
void Distorsion::sethpf (uint8 Phpf)
{
    this->Phpf = Phpf;
...
//==============================================================================
void Distorsion::setPreset (uint8 npreset)
{
    ...
    if (insertion == 0) setParameter (0, (int) (presets[npreset][0] / 1.5)); //lower the volume if this is system effect
    Ppreset = npreset;
    clean();
}
