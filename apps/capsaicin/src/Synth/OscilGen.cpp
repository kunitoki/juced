...
FFTFrequencies OscilGen::outoscilFFTfreqs;
...
void OscilGen::convert2sine (int magtype)
{
    ...
    for (int i = 0; i < MAX_AD_HARMONICS - 1; i++)
    {
        ...
        Phphase[i] = 64 - (int) (64.0 * newphase / double_Pi);
...
REALTYPE OscilGen::basefunc_abssine(REALTYPE x,REALTYPE a){
    ...
    return(sin(pow(x,exp((a-0.5)*5.0))*double_Pi)*2.0-1.0);
};
...
REALTYPE OscilGen::basefunc_sqr(REALTYPE x,REALTYPE a){
    ...
    return(-atan(sin(x*2.0*double_Pi)*a));
};
/*
 * Base Functions - END
 */
...
//    printf("%.5f %.5f\n",basefuncmodulationpar1,basefuncmodulationpar3);
    ...
    switch(Pbasefuncmodulation){
        case 1:t=t*basefuncmodulationpar3+sin((t+basefuncmodulationpar2)*2.0*double_Pi)*basefuncmodulationpar1;//rev
        break;
        case 2:t=t+sin((t*basefuncmodulationpar3+basefuncmodulationpar2)*2.0*double_Pi)*basefuncmodulationpar1;//sine
        break;
        case 3:t=t+pow((1.0-cos((t+basefuncmodulationpar2)*2.0*double_Pi))*0.5,basefuncmodulationpar3)*basefuncmodulationpar1;//power
        break;
    };
    ...
    switch (Pcurrentbasefunc){
        ...
        case 13:smps[i]=basefunc_sqr(t,par);
           break;
        default:smps[i]=-sin(2.0*double_Pi*i/OSCIL_SIZE);
    };
    };
};
...
void OscilGen::oscilfilter(){
    ...
    switch(Pfiltertype){
        ...
        case 10:tmp=pow(5.0,par2*2.0-1.0);
            ...
            gain=cos(par*par*double_Pi/2.0*tmp);//cos
           gain*=gain;
           break;
        case 11:tmp=pow(5.0,par2*2.0-1.0);
            tmp=pow(i/32.0,tmp)*32.0;
            if (Pfilterpar2==64) tmp=i;
            gain=sin(par*par*double_Pi/2.0*tmp);//sin
           gain*=gain;
           break;
        case 12:p2=1.0-par+0.2;
            x=i/(64.0*p2*p2);
             ...
             gain=cos(x*double_Pi)*(1.0-tmp)+1.01+tmp;//low shelf
            break;
...
void OscilGen::modulation(){
    ...
    switch(Pmodulation){
        case 1:t=t*modulationpar3+sin((t+modulationpar2)*2.0*double_Pi)*modulationpar1;//rev
        break;
        case 2:t=t+sin((t*modulationpar3+modulationpar2)*2.0*double_Pi)*modulationpar1;//sine
        break;
        case 3:t=t+pow((1.0-cos((t+modulationpar2)*2.0*double_Pi))*0.5,modulationpar3)*modulationpar1;//power
        break;
    };
...
void OscilGen::prepare()
{
   ...
   for (i=0;i<MAX_AD_HARMONICS;i++) hphase[i]=(Phphase[i]-64.0)/64.0*double_Pi/(i+1);
   ...
   for (i=0;i<MAX_AD_HARMONICS;i++)
   {
      ...
      switch(Phmagtype){
        ...
        default:hmag[i]=1.0-hmagnew; break;
      }
   ...
   if (Pcurrentbasefunc == 0) // sin/cos
   {
       for (i = 0; i < MAX_AD_HARMONICS; i++)
       {
           oscilFFTfreqs.c[i+1] = -hmag[i] * sin (hphase[i] * (i+1)) * 0.5;
           oscilFFTfreqs.s[i+1] = hmag[i] * cos (hphase[i] * (i+1)) * 0.5;
       }
   }
   else
   {
...
void OscilGen::adaptiveharmonic(FFTFrequencies f,REALTYPE freq){
...
short int OscilGen::get (REALTYPE *smps, REALTYPE freqHz, int resonance)
{
    ...
    if ((Prand > 64) && (freqHz >= 0.0) && (!ADvsPAD))
    {
        ...
        rnd = double_Pi * pow ((Prand - 64.0) / 64.0, 2.0);
        for (i = 1; i < nyquist - 1; i++)
        {
    ...
    if ((freqHz > 0.1) && (!ADvsPAD))
    {
        ...
        case 2: power = power * 2.0 - 0.5;
                ...
                REALTYPE rndfreq = 2 * double_Pi * RND;
                for (i = 1; i < nyquist - 1; i++)
                {
...
void OscilGen::addToXML (XmlElement* xml)
{
    xml->setAttribute (T("hmt"), Phmagtype);
    xml->setAttribute (T("bf"), Pcurrentbasefunc);
    xml->setAttribute (T("bfp"), Pbasefuncpar);
    xml->setAttribute (T("bfm"), Pbasefuncmodulation);
    xml->setAttribute (T("bfmp1"), Pbasefuncmodulationpar1);
    xml->setAttribute (T("bfmp2"), Pbasefuncmodulationpar2);
    xml->setAttribute (T("bfmp3"), Pbasefuncmodulationpar3);
    xml->setAttribute (T("mod"), Pmodulation);
    xml->setAttribute (T("modp1"), Pmodulationpar1);
    xml->setAttribute (T("modp2"), Pmodulationpar2);
    xml->setAttribute (T("modp3"), Pmodulationpar3);
    xml->setAttribute (T("wvshp"), Pwaveshaping);
    xml->setAttribute (T("wvshpf"), Pwaveshapingfunction);
    xml->setAttribute (T("flttype"), Pfiltertype);
    xml->setAttribute (T("fltp1"), Pfilterpar1);
    xml->setAttribute (T("fltp2"), Pfilterpar2);
    xml->setAttribute (T("fltbws"), Pfilterbeforews);
    xml->setAttribute (T("spcat"), Psatype);
    xml->setAttribute (T("spcap"), Psapar);
    xml->setAttribute (T("rnd"), Prand);
    xml->setAttribute (T("amprndt"), Pamprandtype);
    xml->setAttribute (T("amprndp"), Pamprandpower);
    xml->setAttribute (T("hshift"), Pharmonicshift);
    xml->setAttribute (T("hshiftf"), Pharmonicshiftfirst);
    xml->setAttribute (T("adpth"), Padaptiveharmonics);
    xml->setAttribute (T("adpthbf"), Padaptiveharmonicsbasefreq);
    xml->setAttribute (T("adpthp"), Padaptiveharmonicspower);

    {
    XmlElement* e = new XmlElement (T("spct"));
    for (int n = 0; n < MAX_AD_HARMONICS; n++)
    {
        if (Phmag[n] == 64 && Phphase[n] == 64) continue;

        XmlElement* h = new XmlElement (T("h") + String (n));
        h->setAttribute (T("m"), Phmag[n]);
        h->setAttribute (T("p"), Phphase[n]);
        e->addChildElement (h);
    }
    ...
    if (Pcurrentbasefunc == 127)
    {
        ...
        {
        XmlElement* e = new XmlElement (T("basef"));
        for (int i = 1; i < OSCIL_SIZE / 2; i++)
        {
            ...
            {
                XmlElement* bf = new XmlElement (T("bfh"));
                bf->setAttribute (T("cos"), xc);
                bf->setAttribute (T("sin"), xs);
                e->addChildElement (bf);
            }
        }
        xml->addChildElement (e);
        }
...
void OscilGen::updateFromXML (XmlElement* xml)
{
    Phmagtype = xml->getIntAttribute (T("hmt"), Phmagtype);
    Pcurrentbasefunc = xml->getIntAttribute (T("bf"), Pcurrentbasefunc);
    Pbasefuncpar = xml->getIntAttribute (T("bfp"), Pbasefuncpar);
    Pbasefuncmodulation = xml->getIntAttribute (T("bfm"), Pbasefuncmodulation);
    Pbasefuncmodulationpar1 = xml->getIntAttribute (T("bfmp1"), Pbasefuncmodulationpar1);
    Pbasefuncmodulationpar2 = xml->getIntAttribute (T("bfmp2"), Pbasefuncmodulationpar2);
    Pbasefuncmodulationpar3 = xml->getIntAttribute (T("bfmp3"), Pbasefuncmodulationpar3);
    Pmodulation = xml->getIntAttribute (T("mod"), Pmodulation);
    Pmodulationpar1 = xml->getIntAttribute (T("modp1"), Pmodulationpar1);
    Pmodulationpar2 = xml->getIntAttribute (T("modp2"), Pmodulationpar2);
    Pmodulationpar3 = xml->getIntAttribute (T("modp3"), Pmodulationpar3);
    Pwaveshaping = xml->getIntAttribute (T("wvshp"), Pwaveshaping);
    Pwaveshapingfunction = xml->getIntAttribute (T("wvshpf"), Pwaveshapingfunction);
    Pfiltertype = xml->getIntAttribute (T("flttype"), Pfiltertype);
    Pfilterpar1 = xml->getIntAttribute (T("fltrp1"), Pfilterpar1);
    Pfilterpar2 = xml->getIntAttribute (T("fltrp2"), Pfilterpar2);
    Pfilterbeforews = xml->getIntAttribute (T("fltbwvshp"), Pfilterbeforews);
    Psatype = xml->getIntAttribute (T("spcat"), Psatype);
    Psapar = xml->getIntAttribute (T("spcap"), Psapar);
    Prand = xml->getIntAttribute (T("rnd"), Prand);
    Pamprandtype = xml->getIntAttribute (T("amprndt"), Pamprandtype);
    Pamprandpower = xml->getIntAttribute (T("amprndp"), Pamprandpower);
    Pharmonicshift = xml->getIntAttribute (T("hshift"), Pharmonicshift); // -64 64
    Pharmonicshiftfirst = xml->getIntAttribute (T("hshiftf"), Pharmonicshiftfirst);
    Padaptiveharmonics = xml->getIntAttribute (T("adpth"), Padaptiveharmonics); // 0 127
    Padaptiveharmonicsbasefreq = xml->getIntAttribute (T("adpthbf"), Padaptiveharmonicsbasefreq); // 0 255
    Padaptiveharmonicspower = xml->getIntAttribute (T("adpthp"), Padaptiveharmonicspower); // 0 200

    {
    XmlElement* e = xml->getChildByName (T("spct"));
    if (e)
    {
        for (int n = 0; n < MAX_AD_HARMONICS; n++)
        {
            ...
            XmlElement* h = e->getChildByName (T("h") + String (n));
            if (h)
            {
                Phmag[n] = h->getIntAttribute (T("m"), 64);
                Phphase[n] = h->getIntAttribute (T("p"), 64);
            }
        }
        prepare ();
    }
    ...
    {
    XmlElement* e = xml->getChildByName (T("basef"));
    if (e)
    {
        int i = 1;

        forEachXmlChildElement (*e, k)
        {
            if (i >= OSCIL_SIZE / 2) break;
            if (k->hasTagName (T("bfh")))
            {
                basefuncFFTfreqs.c[i] = xml->getDoubleAttribute (T("cos"), 0.0);
                basefuncFFTfreqs.s[i] = xml->getDoubleAttribute (T("sin"), 0.0);
                i++;
            }
        }
