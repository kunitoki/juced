 ...
 */
void Resonance::applyres(int n,FFTFrequencies fftdata,REALTYPE freq){
...
void Resonance::interpolatepeaks(int type){
	...
	if ((Prespoints[i]!=64)||(i+1==N_RES_POINTS)){
		...
		if (type==0) x=(1-cos(x*double_Pi))*0.5;
...
void Resonance::addToXML (XmlElement* xml)
{
    xml->setAttribute (T("on"), Penabled);
    xml->setAttribute (T("maxdb"), PmaxdB);
    xml->setAttribute (T("cntfrq"), Pcenterfreq);
    xml->setAttribute (T("octfrq"), Poctavesfreq);
    xml->setAttribute (T("pff"), Pprotectthefundamental);
    xml->setAttribute (T("respnts"), N_RES_POINTS);
    for (int i = 0; i < N_RES_POINTS; i++)
    {
        XmlElement* e = new XmlElement (T("p"));
        e->setAttribute (T("v"), Prespoints[i]);
        xml->addChildElement (e);
    }
}

void Resonance::updateFromXML (XmlElement* xml)
{
    Penabled = xml->getIntAttribute (T("on"), Penabled);
    PmaxdB = xml->getIntAttribute (T("maxdb"), PmaxdB);
    Pcenterfreq = xml->getIntAttribute (T("cntfrq"), Pcenterfreq);
    Poctavesfreq = xml->getIntAttribute (T("octfrq"), Poctavesfreq);
    Pprotectthefundamental = xml->getIntAttribute (T("pff"), Pprotectthefundamental);

    int i = 0;
    forEachXmlChildElement (*xml, e)
    {
        if (i >= N_RES_POINTS) break;
        if (e->hasTagName (T("p")))
            Prespoints[i] = e->getIntAttribute (T("v"), Prespoints[i]);
        i++;
    }
}
