...
void ADnote::initparameters()
{
    ...
    NoteGlobalPar.Volume =
        4.0 * pow (0.1, 3.0 * (1.0 - partparams->GlobalPar.PVolume / 96.0)) // -60 dB .. 0 dB
    ...
    for (nvoice = 0; nvoice < NUM_VOICES; nvoice++)
    {
        ...
        if (partparams->VoicePar[nvoice].PFreqLfoEnabled!=0)
            NoteVoicePar[nvoice].FreqLfo=new LFO(partparams->VoicePar[nvoice].FreqLfo,basefreq);
        ...
        if ((NoteVoicePar[nvoice].FMEnabled!=NONE)&&(NoteVoicePar[nvoice].FMVoice<0))
        {
            ...
            for (int i=0;i<OSCIL_SMP_EXTRA_SAMPLES;i++)
                NoteVoicePar[nvoice].FMSmp[OSCIL_SIZE+i]=NoteVoicePar[nvoice].FMSmp[i];
...
void ADnote::computecurrentparameters()
{
    ...
    REALTYPE tmpfilterfreq = globalfilterpitch
    ...
    for (nvoice = 0; nvoice < NUM_VOICES; nvoice++)
    {
        ...
        newamplitude[nvoice] = pow (0.1, 3.0 * (1.0 - partparams->VoicePar[nvoice].PVolume / 127.0)); // 1.0
        ...
        if (NoteVoicePar[nvoice].noisetype == 0)
        {
            ...
            if (NoteVoicePar[nvoice].FreqLfo != NULL)
                voicepitch += NoteVoicePar[nvoice].FreqLfo->lfoout() / 100.0 * ctl->bandwidth.relbw;
...
inline void ADnote::fadein (REALTYPE *smps)
{
    ...
    {
        //fade-in
        REALTYPE tmp = 0.5 - cos ((REALTYPE) i / (REALTYPE) n * double_Pi) * 0.5;
        smps[i] *= tmp;
    }
}
...
int ADnote::noteout (REALTYPE *outl, REALTYPE *outr)
{
    ...
    for (nvoice = 0; nvoice < NUM_VOICES; nvoice++)
    {
        ...
        if (NoteVoicePar[nvoice].noisetype == 0)
        {
            ...
            default:
#ifdef WAVETABLE_CUBIC_INTERPOLATION
                               ComputeVoiceOscillator_CubicInterpolation (nvoice);
#else
                               ComputeVoiceOscillator_LinearInterpolation (nvoice);
#endif
            }
        }
        else // voice mode = noise
        ...
        if (ABOVE_AMPLITUDE_THRESHOLD (oldamplitude[nvoice], newamplitude[nvoice]))
        {
            ...
            // test if the amplitude is raising and the difference is high
            if ((newamplitude[nvoice] > oldamplitude[nvoice])
                && ((newamplitude[nvoice] - oldamplitude[nvoice]) > 0.25))
            {
                rest = 16; // SOUND_BUFFER_SIZE / 16;
            ...
            for (i = 0; i < rest; i++)
            {
                tmpwave [i + (SOUND_BUFFER_SIZE - rest)] *= INTERPOLATE_AMPLITUDE (oldamplitude[nvoice], newamplitude[nvoice], i, rest);
            }
        }
        else
            for (i = 0; i < SOUND_BUFFER_SIZE; i++)
        ...
        // Filter
        if (NoteVoicePar[nvoice].VoiceFilter != NULL)
            NoteVoicePar[nvoice].VoiceFilter->filterout (&tmpwave[0], SOUND_BUFFER_SIZE);
    ...
    NoteGlobalPar.GlobalFilterL->filterout (&outl[0], SOUND_BUFFER_SIZE);
    ...
    if (stereo == 0)
    {
        ...
        NoteGlobalPar.GlobalFilterR->filterout (&outr[0], SOUND_BUFFER_SIZE);
...
void ADnote::relasekey()
{
    ...
    {
        if (NoteVoicePar[i].Enabled == 0) continue;
        if (NoteVoicePar[i].AmpEnvelope)    NoteVoicePar[i].AmpEnvelope->relasekey();
        if (NoteVoicePar[i].FreqEnvelope)   NoteVoicePar[i].FreqEnvelope->relasekey();
        if (NoteVoicePar[i].FilterEnvelope) NoteVoicePar[i].FilterEnvelope->relasekey();
        if (NoteVoicePar[i].FMFreqEnvelope) NoteVoicePar[i].FMFreqEnvelope->relasekey();
        if (NoteVoicePar[i].FMAmpEnvelope)  NoteVoicePar[i].FMAmpEnvelope->relasekey();
    }
