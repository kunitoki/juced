...
static long SR2_(zoh_process) (SR2_(SRC_PRIVATE) *psrc, SR2_(SRC_DATA) *data) ;
static void SR2_(zoh_reset) (SR2_(SRC_PRIVATE) *psrc) ;
...
{	SR2_(ZOH_DATA) 	*zoh ;
	...
	zoh = (SR2_(ZOH_DATA)*) psrc->private_data ;
...
{	SR2_(ZOH_DATA) *zoh = NULL ;
	...
	if (psrc->private_data == NULL)
	{	zoh = calloc (1, sizeof (*zoh) + psrc->channels * sizeof (sr2_float_t)) ;
		if (zoh == NULL)
