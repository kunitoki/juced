...
**	Double precision floating polong is not accurate enough so use a 64 bit
**	fixed polong value instead. SHIFT_BITS (current value of 48) is the number
**	of bits to the right of the decimal point.
**	The rest of the macros are for retrieving the fractional and integer parts
**	and for converting floats and ints to the fixed polong format or from the
**	fixed polong type back to integers and floats.
*/
...
typedef struct
{	long 	sinc_magic_marker ;

	long 	channels ;
	long	in_count, in_used ;
	long	out_count, out_gen ;

	long 	coeff_half_len, index_inc ;
	long 	has_diffs ;
	...
	long 	b_current, b_end, b_real_end, b_len ;
	sr2_float_t	buffer [1] ;
} SR2_(SINC_FILTER) ;

static long SR2_(sinc_process) (SR2_(SRC_PRIVATE) *psrc, SR2_(SRC_DATA) *data) ;

static double SR2_(calc_output) (SR2_(SINC_FILTER) *filter, increment_t increment, increment_t start_filter_index, long ch) ;

static void SR2_(prepare_data) (SR2_(SINC_FILTER) *filter, SR2_(SRC_DATA) *data, long half_filter_chan_len) ;

static void SR2_(sinc_reset) (SR2_(SRC_PRIVATE) *psrc) ;
...
{	SR2_(SINC_FILTER) *filter, temp_filter ;
	long count, bits ;
	...
	{	filter = (SR2_(SINC_FILTER)*) psrc->private_data ;
	...
	psrc->process = SR2_(sinc_process) ;
	psrc->reset = SR2_(sinc_reset) ;
	...
	** a better way. Need to look at SR2_(prepare_data) () at the same time.
	*/

	temp_filter.b_len = 1000 + 2 * lrint (0.5 + temp_filter.coeff_len / (temp_filter.index_inc * 1.0) * SRC_MAX_RATIO) ;
	temp_filter.b_len *= temp_filter.channels ;

	if ((filter = calloc (1, sizeof (SR2_(SINC_FILTER)) + sizeof (filter->buffer [0]) * (temp_filter.b_len + temp_filter.channels))) == NULL)
	...
	*filter = temp_filter ;
	...
	SR2_(sinc_reset) (psrc) ;

	count = filter->coeff_half_len ;
	for (bits = 0 ; (1 << bits) < count ; bits++)
		count |= (1 << bits) ;

	if (bits + SHIFT_BITS - 1 >= (long) (sizeof (increment_t) * 8))
...
static void
SR2_(sinc_reset) (SR2_(SRC_PRIVATE) *psrc)
{	SR2_(SINC_FILTER) *filter ;
...
{	SR2_(SINC_FILTER) *filter ;
	...
	long 		half_filter_chan_len, samples_in_hand, ch ;
	...
	while (filter->out_gen < filter->out_count)
	{
		...
		{	SR2_(prepare_data) (filter, data, half_filter_chan_len) ;
		...
		{	data->data_out [filter->out_gen] = (float_increment / filter->index_inc) *
											SR2_(calc_output) (filter, increment, start_filter_index, ch) ;
			filter->out_gen ++ ;
			} ;
...
static void
SR2_(prepare_data) (SR2_(SINC_FILTER) *filter, SR2_(SRC_DATA) *data, long half_filter_chan_len)
{	long len = 0 ;
...
static double
SR2_(calc_output) (SR2_(SINC_FILTER) *filter, increment_t increment, increment_t start_filter_index, long ch)
{	double		fraction, left, right, icoeff ;
	increment_t	filter_index, max_filter_index ;
	long 		data_index, coeff_count, indx ;
