...
void FV3_(firwindow)::Hamming(fv3_float_t w[], const long N)
{
  ...
  for(i = 0; i < N; i++)
    {
      w[i] = 0.54 - (0.46*cos(2.0*M_PI*(fv3_float_t)i/M));
    }
  return;
}
...
void FV3_(firwindow)::Hanning(fv3_float_t w[], const long N)
{
  ...
  for(i = 0; i < N; i++)
    {
      w[i] = 0.5 * (1.0 - cos(2.0*M_PI*(fv3_float_t)i/M));
    }
  return;
}
...
void FV3_(firwindow)::Blackman(fv3_float_t w[], const long N)
{
  ...
  for(i = 0; i < N; i++)
    {
      w[i] =
	0.42 - (0.5 * cos(2.0*M_PI*(fv3_float_t)i/M)) +
	(0.08*cos(4.0*M_PI*(fv3_float_t)i/M));
    }
  return;
}
...
fv3_float_t FV3_(firwindow)::KaiserBeta(const fv3_float_t dB)
{
  ...
  if(dB > 50)
    ...
    beta = 0.5842*pow(dB-21, 0.4) + 0.07886*(dB-21);
  else
    beta = 0; 
  return beta;
}
...
void FV3_(firwindow)::CosROW(fv3_float_t w[], const long N, const fv3_float_t fc,
			  const fv3_float_t alpha)
{
  long i;
  const fv3_float_t M = N-1;
  fv3_float_t n;
  for(i = 0; i < N; i++)
    {
      ...
      w[i] = cos(M_PI*n*2.0*fc*M_PI*alpha)
	/ (1 - pow((M_PI*n*2.0*fc*2*alpha),2));
    }        
  return;
}
...
void FV3_(firwindow)::Sinc(fv3_float_t sinc[], const long N, const fv3_float_t fc)
{
  ...
  for(i = 0; i < N; i++)
	...
	{
	  n = (fv3_float_t)i - M/2.0;
	  sinc[i] = sin(M_PI*n*2.0*fc) / (M_PI*n);
	}
    }        
  return;
}
