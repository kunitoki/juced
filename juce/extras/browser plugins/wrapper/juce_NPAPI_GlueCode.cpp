...
#if (JUCE_MAC || JUCE_LINUX) && JUCE_DEBUG && 1
static void log (const String& s)
{
#if JUCE_MAC
    FILE* f = fopen ("/Users/jules/Desktop/log.txt", "a+");
#elif JUCE_LINUX
    FILE* f = fopen ("/home/INSULA/lucio/Desktop/log.txt", "a+");
#endif
...
//==============================================================================
#elif JUCE_LINUX
class BrowserMessageThread : public Thread
{
public:
    BrowserMessageThread()
      : Thread (T("BrowserMessageThread"))
    {
        startThread (7);
    }
    ...
    juce_DeclareSingleton (BrowserMessageThread, false)
};

juce_ImplementSingleton (BrowserMessageThread)
...
public:
    ...
    //==============================================================================
#elif JUCE_LINUX
    Window currentParentView;

    void resizeToParentWindow(Display* display)
    {
        if (currentParentView != 0)
        {
            XWindowAttributes attr;
            if (XGetWindowAttributes (display, currentParentView, &attr) == True)
            {
                setBounds (0, 0, attr.width, attr.height);
            }
        }
    }

    Window findParentWindowAt (Display* display, Window window, float x, float y) const
    {
#if 0
        Window root, parent;
        int winx, winy;
        ...
        if (XQueryTree (display, 
                        window,
                        &root, &parent,
                        &children, &nchildren) == True)
#endif
        {
            printf ("findParentWindowAt %d \n", (int) child);
            return parent;
        }
        else
        {
            printf ("findParentWindowAt failed \n");
        }
...
public:
    ...
    {
        Window parentView = 0;
        Window windowRef = window != 0 ? ((Window) window->window) : 0;
        Display* display = window != 0 ? ((Display*) ((NPSetWindowCallbackStruct *)window->ws_info)->display) : 0;

        if (windowRef != 0)
        {
            parentView = findParentWindowAt (display, windowRef, window->x + 0.5f, window->y + 0.5f);
        }
        ...
        if (parentView != currentParentView)
        {
            ...
            if (parentView != 0)
            {
                ...
                Window editorView = (Window) getWindowHandle();
                XReparentWindow (display, editorView, parentView, 0, 0);

                resizeToParentWindow (display);
                setVisible (true);
            }
        }
...
#if JUCE_LINUX
        BrowserMessageThread::getInstance ();
#endif
    }
...
NPError NPP_Destroy (NPP npp, NPSavedData** save)
{
    ...
    if (p != 0)
    {
        ...
        {
#if JUCE_LINUX
            BrowserMessageThread::deleteInstance ();
#endif
