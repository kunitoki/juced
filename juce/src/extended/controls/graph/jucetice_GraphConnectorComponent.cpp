...
DBG ("START >>> GraphConnectorComponent::removeLinkWith (GraphConnectorComponent* connector)");
    ...
    std::set<GraphLinkComponent*>::iterator linkIter;

    for (linkIter = links.begin(); linkIter != links.end(); linkIter++)
    {
        if ((*linkIter)->from == connector || (*linkIter)->to == connector)
        {
            if (notifyListeners && parentGraph)
                parentGraph->notifyLinkDisconnected (*linkIter);
...
DBG ("START >>> GraphConnectorComponent::removeLinkWith (GraphNodeComponent* node)");
    ...
    for (linkIter = links.begin(); linkIter != links.end(); linkIter++)
    {
        if (((*linkIter)->from && (*linkIter)->from->getParentGraphComponent () == node)
            || ((*linkIter)->to && (*linkIter)->to->getParentGraphComponent () == node))
        {
            if (notifyListeners && parentGraph)
                parentGraph->notifyLinkDisconnected (*linkIter);
...
void GraphConnectorComponentInput::updatePosition()
{
    ...
    for (linkIter = links.begin(); linkIter != links.end(); linkIter++)
    {
        (*linkIter)->setEndPoint (centre_x, centre_y);
    }
}
...
void GraphConnectorComponentOutput::updatePosition()
{
    ...
    for (linkIter = links.begin(); linkIter != links.end(); linkIter++)
    {
        (*linkIter)->setStartPoint (centre_x, centre_y);
    }
}
