...
#include "jucetice_ViewportNavigator.h"
#include "../../../gui/graphics/imaging/juce_ImageFileFormat.h"

//==============================================================================
ViewportNavigator::ViewportNavigator (Viewport* const viewportToWatch)
    : watchedViewport (0),
      componentSnapshot (0),
      viewPositionX (0),
      viewPositionY (0),
...
ViewportNavigator::~ViewportNavigator ()
{
    ...
    deleteAndZero (componentSnapshot);
}
...
//==============================================================================
Viewport* ViewportNavigator::getViewedViewport ()
{
    return watchedViewport;
}

void ViewportNavigator::setViewedViewport (Viewport* const viewportToWatch)
{
    bool needsUpdate = false;

    if (watchedViewport && watchedViewport->isValidComponent ())
    {
        Component* comp = watchedViewport->getViewedComponent ();
        if (comp && comp->isValidComponent ())
...
void ViewportNavigator::viewedComponentChanged (Viewport* const viewportThatChanged,
                                                Component* const newViewedComponent)
{
    updateVisibleArea ();
}

void ViewportNavigator::visibleAreaChanged (Viewport* const viewportThatChanged,
...
void ViewportNavigator::updateSnapshot ()
{
        ...
        && getWidth () > 0 && getHeight () > 0)
    {
        ...
        if (comp)
        {
            ...
            Rectangle areaToGrab;
            areaToGrab.setSize (comp->getWidth (), comp->getHeight ());

            Image* componentSnapshotCopy = comp->createComponentSnapshot (areaToGrab, false);
            componentSnapshot = componentSnapshotCopy->createCopy (getWidth (), getHeight ());
            deleteAndZero (componentSnapshotCopy);

            if (oldSnapshot)
                deleteAndZero (oldSnapshot);
        }
    }
}
