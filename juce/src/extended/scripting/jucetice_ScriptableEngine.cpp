...
void ScriptableEngine::executeFunction (const String& functionName)
{
    ...
    if (currentEngine != 0 && status == Compiled)
    {
    	...
    	if (func_id < 0)
    	{
    	    printErrorMessage (T("Runtime"), String("Cannot find function ") + functionName, T(""), -1);
    		return;
    	}
	    ...
	    if (! context)
	    {
		    printErrorMessage (T("Runtime"), T("Cannot create context for execution"), T(""), -1);
		    return;
	    }
        ...
        if (r < 0)
        {
	        printErrorMessage (T("Runtime"), T("Cannot prepare function ececution"), T(""), -1);
	        return;
        }
...
bool ScriptableEngine::executeFunction (const String& strFunction)
{
	...
	if (functionID < 0)
	{
	    reportErrors (T("runtime"), String("Cannot find function ") + strFunction, T(""), -1);
		return false;
	}
	...
	if (context == 0)
	{
		reportErrors (T("runtime"), T("Cannot create context for execution"), T(""), -1);
		return false;
	}
	...
	if (r < 0)
	{
		reportErrors (T("runtime"), T("Cannot prepare function ececution"), T(""), -1);
		return false;
	}
