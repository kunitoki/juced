...
public:
          ...
          sampleFormat (AudioDataConverters::int16LE)
    {
        failed (snd_pcm_open (&handle, id,
                              ...
                              SND_PCM_ASYNC), T("snd_pcm_open"));
    }

    ~ALSADevice()
    {
        if (handle != 0)
            failed (snd_pcm_close (handle), T("snd_pcm_close"));
    }
    ...
    bool setParameters (unsigned int sampleRate, int numChannels, int bufferSize)
    {
        ...
        if (failed (snd_pcm_hw_params_set_rate_near (handle, hwParams, &sampleRate, 0), T("snd_pcm_hw_params_set_rate_near"))
            || failed (snd_pcm_hw_params_set_channels (handle, hwParams, numChannels), T("snd_pcm_hw_params_set_channels"))
            || failed (snd_pcm_hw_params_set_periods_near (handle, hwParams, &periods, &dir), T("snd_pcm_hw_params_set_periods_near"))
            || failed (snd_pcm_hw_params_set_period_size_near (handle, hwParams, &samplesPerPeriod, &dir), T("snd_pcm_hw_params_set_period_size_near"))
            || failed (snd_pcm_hw_params (handle, hwParams), T("snd_pcm_hw_params")))
        {
            return false;
        }
        ...
        if (failed (snd_pcm_sw_params_current (handle, swParams), T("snd_pcm_sw_params_current"))
            || failed (snd_pcm_sw_params_get_boundary (swParams, &boundary), T("snd_pcm_sw_params_get_boundary"))
            || failed (snd_pcm_sw_params_set_silence_threshold (handle, swParams, 0), T("snd_pcm_sw_params_set_silence_threshold"))
            || failed (snd_pcm_sw_params_set_silence_size (handle, swParams, 0), T("snd_pcm_sw_params_set_silence_size"))
            || failed (snd_pcm_sw_params_set_start_threshold (handle, swParams, samplesPerPeriod), T("snd_pcm_sw_params_set_start_threshold"))
            || failed (snd_pcm_sw_params_set_stop_threshold (handle, swParams, boundary), T("snd_pcm_sw_params_set_stop_threshold"))
            || failed (snd_pcm_sw_params (handle, swParams), T("snd_pcm_sw_params")))
        {
            return false;
        }
    ...
    bool write (float** const data, const int numSamples)
    {
        if (isInterleaved)
        {
            ...
            if (failed (num, T("snd_pcm_writei")) && num != -EPIPE && num != -ESTRPIPE)
                return false;
        }
        else
        {
            ...
            if (failed (num, T("snd_pcm_writen")))
            {
                if (num == -EPIPE)
                {
                    if (failed (snd_pcm_prepare (handle), T("snd_pcm_prepare")))
                        return false;
                }
    ...
    bool read (float** const data, const int numSamples)
    {
        ...
        else
        {
            ...
            if (failed (num, T("snd_pcm_readn")) && num != -EPIPE && num != -ESTRPIPE)
                return false;
...
private:
    ...
    //==============================================================================
    bool failed (const int errorNum, const String& when = String::empty)
    {
        if (errorNum >= 0)
            return false;

        error = snd_strerror (errorNum);
        DBG (T("ALSA error (") + when + T("): ") + error + T("\n"));
        return true;
    }
};
