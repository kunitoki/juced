...
public:
    ...
    */
    ...
    /** Makes a component visible using a groovy fade-in and animated zoom effect.

        To do this, this function will cunningly:
            - take a snapshot of the component as it will look when showed
            - replace it with a special component that will continue drawing the
              snapshot, animating it and gradually making it less transparent
            - call setVisible(false) on the component

        Be careful not to delete your component before showing it.

        Passing non-zero x and y values will cause the ghostly component image to
        also whizz in by this distance while fading in. If the scale factor is
        not 1.0, it will also zoom from this new size to the component's size that will
        be after.
    */
    void fadeInComponent (const int lengthOfFadeInMilliseconds,
                          const int deltaXToMove = 0,
                          const int deltaYToMove = 0,
                          const float scaleFactorAtStart = 1.0f);
